...
The qualitative ordering of final genotypic diversity---flat $<$ island $<$ hourglass---holds for mazes ($0.27 < 0.32 < 0.46$) and symbolic regression ($5.71 < 5.83 < 7.07$). Checkers deviates: flat and hourglass tie at $0.30$, with island slightly higher at $0.34$ (ordering: flat $=$ hourglass $<$ island). This deviation is expected---the checkers fitness landscape saturates rapidly (all strategies achieve perfect 1.0 win rate), compressing the narrow fitness range into a regime where diversity differences between composition patterns are attenuated. The maze domain, with its richer fitness landscape, shows the clearest separation.

Checkers compresses diversity differences because the fitness landscape saturates quickly---all three strategies achieve perfect 1.0 win rate against the baseline opponents, leaving no selective gradient to differentiate the strategies. Phenotypic diversity collapses to 0.0 in all three checkers conditions. This is itself informative: on saturating landscapes, composition pattern matters less because all strategies reach the same basin.

The island model consistently maintains intermediate diversity. In mazes, where the landscape is multimodal, island achieves both the highest fitness (0.73 vs.\ 0.70 flat, 0.68 hourglass) and higher diversity than flat. The hourglass strategy produces the highest final genotypic diversity (0.46 in mazes) but sacrifices fitness (0.68), consistent with the diversity-quality tradeoff predicted by the spike-crash-rebound fingerprint.

The key result is that \emph{on non-saturating landscapes, composition structure is the invariant}. Binary strings and expression trees---where fitness gradients persist throughout the run---respond to the same composition pattern in the same qualitative way. Checkers, where the landscape saturates early (all strategies reach perfect 1.0 win rate), compresses the fingerprint differences: flat and hourglass converge to the same diversity level, and the trajectory-shape distinctions that are clear in mazes and symbolic regression are attenuated. The fingerprint follows the functor, not the object, provided the fitness landscape maintains sufficient selective gradient.
...
\paragraph{Spectral background.}
For readers unfamiliar with spectral graph theory: the Laplacian of a graph has eigenvalues $0 = \lambda_1 \leq \lambda_2 \leq \cdots \leq \lambda_n$. The first eigenvalue $\lambda_1 = 0$ is the same for every connected graph---its eigenvector is the all-ones vector representing the uniform stationary distribution, and it simply confirms connectivity. All the interesting structure lives in the second eigenvalue $\lambda_2$, the \emph{algebraic connectivity} (also called the Fiedler value). It measures how difficult the graph is to ``cut''---how hard you would have to work to separate it into two poorly-connected halves. High $\lambda_2$ means the graph is well-connected and hard to isolate into parts; low $\lambda_2$ means it is loosely connected and easy to partition. Under a random-walk model of migration, $\lambda_2$ governs the mixing time: the number of steps until a walker's position becomes approximately uniform across all islands. Small $\lambda_2$ means slow mixing---islands stay genetically differentiated for a long time, sustaining high diversity. Large $\lambda_2$ means fast mixing---islands rapidly converge to the same gene pool, collapsing diversity. The diversity fingerprint ordering we observe in the experiments is therefore the \emph{inverse} of the $\lambda_2$ ordering: the topologies that mix fastest (high $\lambda_2$) produce the lowest sustained diversity, and vice versa.
...
This result has direct consequences for distributed GA implementations. Any system that checkpoints and restarts island-model runs, composes partial runs from different compute nodes, or chains island-model phases in a multi-stage optimization pipeline must account for the laxity. The composed result will differ from continuous execution by a characteristic magnitude $D^*$---not by a small perturbation, but by a population-level divergence of ${\sim}0.78$ in normalized terms. For applications where genotypic reproducibility matters (e.g., evolved safety-critical controllers), this is a dealbreaker for naive checkpointing. For applications where only fitness matters, the laxity is benign: fitness divergence is 0.0 across all conditions tested. The multi-seed experiment (Table~\ref{tab:multiseed}) confirms this at scale: both strict and lax compositions reach the OneMax optimum, yet their diversity trajectories diverge with effect sizes exceeding $d = 4.0$ from generation~50 onward. This pattern echoes findings in LLM-based multi-agent systems, where independent (lax) composition amplifies errors $17.2\times$ versus $4.4\times$ for centralized (strict) composition~\cite{chen2025scaling}.

\subsection{Evolved Artifacts}

To ground the fingerprint results in concrete outputs, we briefly examine what each domain's GA produced and how artifact quality connects to composition pattern.

\paragraph{Checkers.}
The hourglass strategy (20~generations) evolved weight vectors that beat hand-tuned defaults 20--0. The evolved strategy prioritizes vulnerability assessment (weight 2.14) over material advantage (1.53)---threat awareness over piece counting. Since all strategies saturate to perfect fitness on this domain, the fingerprint differences do not translate into artifact quality differences.

\paragraph{Mazes.}
The island strategy (20~generations) produced the highest-fitness mazes (0.73 vs.\ 0.70 flat, 0.68 hourglass). The island model's diversity maintenance fingerprint---stable intermediate diversity rather than monotonic decline---explains this result: by preserving a broader pool of structural variants across subpopulations, the island strategy explores more of the maze design space and finds higher-quality solutions. The best evolved maze has a path length of 18 cells in a $6 \times 6$ grid with 8 dead ends and a branching factor of 2.3.

\paragraph{Symbolic regression.}
All four strategies recover the exact target expression $(x \cdot x) + x$. The fingerprint differences manifest not in best fitness but in population composition: final phenotypic diversity ranges from 213.42 (adaptive) to 615.42 (island), a $2.9\times$ spread. The island strategy's stable-maintenance fingerprint preserves a diverse pool of near-optimal expressions, while the adaptive strategy's spike-then-collapse fingerprint eliminates all alternatives.
...
Three domains demonstrate that fingerprints are not domain-specific, but three is not exhaustive. Extending the framework to permutation-based domains (TSP, scheduling), variable-length genomes (NEAT-style neuroevolution~\cite{eiben2015introduction}), and high-dimensional continuous optimization (CMA-ES-scale problems) would strengthen the universality claim. We expect the fingerprint patterns to hold---they are composition-determined---but empirical confirmation is needed.

\paragraph{Formal proofs.}
The Strict/Lax Dichotomy (Theorem~\ref{thm:dichotomy}) is stated with formal precision and supported by experimental evidence across migration frequencies and boundary positions. Full categorical proofs, including the connection between laxity magnitude $D^*$ and the spectral gap of the migration topology's Markov chain, are developed in an extended version of this work. The present paper focuses on the empirical phenomenon and its practical consequences.
...
Composition determines behavior. We have demonstrated this across three domains with three results: diversity fingerprints are stable signatures of composition pattern, not of individual operators or genome types; the island functor exhibits a sharp strict/lax dichotomy at zero migration---confirmed across 30~independent seeds with Cohen's $d > 4.0$ ($p < 10^{-10}$)---with practical consequences for checkpointing and distributed execution; and three levels of composition---operators, pipelines, strategies---share the same Kleisli algebraic structure, giving a recursive design algebra for genetic algorithms.
